...
export class SplunkClient {
  ...
  private log(emoji: string, message: string, level: 'info' | 'debug' | 'error' = 'info') {
    const timestamp = new Date().toISOString();
    console[level === 'error' ? 'error' : 'log'](`${timestamp} - ${emoji} ${message}`);
  }
  ...
  async searchSplunk(
    ...
    if (!strippedQuery.startsWith('|') && !strippedQuery.toLowerCase().startsWith('search')) {
    ...
    try {
      ...
      // Get results
      const resultsResponse = await this.client.get(`/services/search/jobs/${jobSid}/results`, {
        params: {
          output_mode: 'json',
          count: maxResults,
        },
  ...
  async getIndexInfo(indexName: string): Promise<SplunkIndex> {
    try {
      const response = await this.client.get(`/services/data/indexes/${indexName}`, {
        params: { output_mode: 'json' },
      });
    ...
    try {
      ...
      for (const entry of response.data.entry || []) {
        try {
          ...
          this.log('⚠️', `Error processing saved search: ${error.message}`, 'info');
        }
      }

      return savedSearches;
    } catch (error: any) {
      this.log('❌', `Failed to list saved searches: ${error.message}`, 'error');
      throw error;
    }
  }
  ...
  async getCurrentUser(): Promise<SplunkUser> {
    try {
      ...
      try {
        ...
        if (contextData.entry && contextData.entry.length > 0) {
          ...
          if (username) {
            ...
            this.log('🔍', `Using username from current-context: ${currentUsername}`, 'debug');
          }
        }
      } catch (error: any) {
        this.log('⚠️', `Could not get username from current-context: ${error.message}`, 'info');
      }

      // Get user information
      const userResponse = await this.client.get(`/services/authentication/users/${currentUsername}`, {
        params: { output_mode: 'json' },
      });
      ...
      const roles = Array.isArray(content.roles) ? content.roles : content.roles ? [content.roles] : [];
      const capabilities = Array.isArray(content.capabilities) ? content.capabilities : [];

      this.log('✅', `Successfully retrieved current user information: ${currentUsername}`, 'info');
    ...
    try {
      ...
      for (const entry of response.data.entry || []) {
        try {
          ...
          this.log('⚠️', `Error processing user ${entry.name}: ${error.message}`, 'info');
        }
      }
    ...
    try {
      ...
      try {
        ...
        if (statsResponse.data.entry && statsResponse.data.entry.length > 0) {
          ...
          for (const kvstore of data) {
            try {
              const parsed = typeof kvstore === 'string' ? JSON.parse(kvstore) : kvstore;
              if (parsed.ns && parsed.count !== undefined) {
                collectionStats[parsed.ns] = parsed.count;
              }
            } catch (error: any) {
              this.log('⚠️', `Error parsing KV store stat: ${error.message}`, 'debug');
            }
          }
          this.log('✅', `Retrieved stats for ${Object.keys(collectionStats).length} KV store collections`, 'debug');
        }
      } catch (error: any) {
        this.log('⚠️', `Error retrieving KV store collection stats: ${error.message}`, 'info');
      }

      // Get collections
      const response = await this.client.get('/servicesNS/-/-/storage/collections/config', {
        params: { output_mode: 'json' },
      });
      ...
      for (const entry of response.data.entry || []) {
        try {
          ...
          this.log('✅', `Added collection: ${collectionName} from app: ${app}`, 'debug');
        } catch (error: any) {
          this.log('⚠️', `Error processing collection entry: ${error.message}`, 'info');
        }
      }

      this.log('✅', `Found ${collections.length} KV store collections`, 'info');
      return collections;
    } catch (error: any) {
      this.log('❌', `Error listing KV store collections: ${error.message}`, 'error');
      throw error;
    }
  }
    ...
    try {
      ...
      for (const entry of response.data.entry || []) {
        try {
          ...
          this.log('⚠️', `Error getting info for app ${entry.name}: ${error.message}`, 'info');
        }
      }
      ...
      this.log('✅', `Health check successful. Found ${apps.length} apps`, 'info');
      return result;
    } catch (error: any) {
  ...
  async getIndexesAndSourcetypes(): Promise<IndexesAndSourcetypes> {
    try {
      ...
      const results = await this.searchSplunk(searchQuery, '-24h', 'now', 10000);

      // Process results
      const sourcetypesByIndex: Record<string, Array<{ sourcetype: string; count: string }>> = {};
      ...
      this.log('❌', `Error getting indexes and sourcetypes: ${error.message}`, 'error');
      throw error;
    }
  }
}